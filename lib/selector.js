 ...
 * */
function selector(selector_string) {
    ...
    if (selector_string.search(at) === -1) SelectorError(`css-selector "${selector_string}" must contain ${at}`);
    var
    ...
    for (var s of selectors) {
        ...
        if ($ === rdm) {
            ...
            $ = $.replace(RegExp(attribute_selector, 'g'), match => {
                dict.attribute_selectors.push(match.replace(RegExp(rdm, 'gi'), at)); return '';
            });
            // PsuedoElements
            ...
            $ = $.replace(RegExp(css_class, 'g'), match => {
                dict.css_classes.push(match.replace(RegExp(rdm, 'gi'), at)); return '';
            });
            // ID Selector
            $ = $.replace(RegExp(id_selector, 'g'), match => {
                dict.id_selectors.push(match.replace(RegExp(rdm, 'gi'), at)); return '';
            });
            $ = $.replace(RegExp(regular_class, 'ig'), match => {
                var has_rdm = match.search(rdm) !== -1;
                if (!has_rdm) dict.element_selectors.push(match);
                else dict.regular_classes.push(match.replace(RegExp(rdm, 'g'), at));
                return "";
            });
